...
if(!empty($_GET["password"]) && $_GET["password"] == $password) {
	...
	foreach($data as $item) {
		echo "ID: {$item["id"]}<br>域名: {$item["domain"]}<br>介绍: {$item["description"]}<br><a href=\"?pass={$item["id"]}&password={$_GET["password"]}\">通过</a><br><a href=\"?fail={$item["id"]}&password={$_GET["password"]}\">阻止</a><br><br>";
	}
} else {
